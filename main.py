...
def dashboard():
    ...
    completed_tasks = sum(sum(1 for t in p.tasks if t.completed) for p in plans)
    return render_template('dashboard.html', plans=plans, total_tasks=total_tasks, completed_tasks=completed_tasks)
...
def planner():
    ...
    if request.method == 'POST':
        ...
        plan = {"subjects": subjects_raw, "hours": hours, "tasks": tasks_data}
...
def toggle_task(task_id):
    ...
    db.session.commit()
    return jsonify({'completed': task.completed, 'progress': task.plan.progress})
